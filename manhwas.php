...
// **FIXED**: Removed the login check that caused the redirect loop.
// Now guests can also view this page.
...
$total_items_stmt = $pdo->query("SELECT COUNT(*) FROM manhwas");
...
// --- Fetch Manhwas for the current page ---
$stmt = $pdo->prepare("SELECT * FROM manhwas ORDER BY title ASC LIMIT :limit OFFSET :offset");
...
<head>
    ...
    <title>Manhwa - <?php echo $lang['aether_stream']; ?></title>
...
<header class="main-header">
            ...
            <section class="anime-rows">
                <div class="anime-row">
                    <h2 class="row-title">All Manhwa (A-Z)</h2>
                    <?php if (empty($manhwas)): ?>
                        <p class="no-results">No Manhwa have been added yet.</p>
                    <?php else: ?>
                        <div class="browse-grid">
                            <?php foreach ($manhwas as $manhwa): ?>
                            <a href="/manhwa_chapters?id=<?php echo $manhwa['id']; ?>" class="anime-card">
                                <img src="<?php echo htmlspecialchars($manhwa['cover_image_url']); ?>" alt="<?php echo htmlspecialchars($manhwa['title']); ?>">
                                <div class="card-title"><?php echo htmlspecialchars($manhwa['title']); ?></div>
                            </a>
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
                ...
                <div class="pagination">
                    <?php if ($current_page > 1): ?>
                        <a href="/manhwas?page=<?php echo $current_page - 1; ?>">&laquo; Prev</a>
                    <?php endif; ?>

                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <a href="/manhwas?page=<?php echo $i; ?>" class="<?php echo ($i == $current_page) ? 'active' : ''; ?>"><?php echo $i; ?></a>
                    <?php endfor; ?>
                    
                    <?php if ($current_page < $total_pages): ?>
                        <a href="/manhwas?page=<?php echo $current_page + 1; ?>">Next &raquo;</a>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
